        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {messages.map((msg, index) => (
            <div key={index} className={`flex items-end gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-md lg:max-w-lg p-3 rounded-2xl ${msg.role === 'user' ? 'bg-primary text-white rounded-br-none' : 'bg-stone-200 text-slate-800 rounded-bl-none'}`}>
        ...
        <form onSubmit={handleSubmit} className="p-4 border-t border-stone-200 bg-stone-200/50 rounded-b-2xl">
          <div className="flex items-center bg-white rounded-full p-2 border border-stone-300 focus-within:ring-2 focus-within:ring-primary transition-shadow">
            <input
              ...
              dir="rtl"
            />
            <button type="submit" disabled={isLoading || !input.trim()} className="p-3 bg-primary text-white rounded-full disabled:bg-slate-400 disabled:cursor-not-allowed transition-colors hover:bg-[#31cb86]">
